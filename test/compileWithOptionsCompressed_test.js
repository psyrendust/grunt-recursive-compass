...
exports['recursive-compass'] = {
  ...
  compileWithOptionsCompressed: function(test) {
    ...
    test.equal(compile_actual, compile_expected, "should compile compile.scss with compressed option");

    var foo_actual = grunt.file.read('test/fixtures/test/foo.css');
    var foo_expected = grunt.file.read('test/expected/foo_compressed.css');
    test.equal(foo_actual, foo_expected, "should compile foo.scss with compressed option");
    ...
    test.equal(test_actual, test_expected, "should compile test.scss with compressed option");
